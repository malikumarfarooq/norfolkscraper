...
class BulkFetchParcelDataJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach ($this->propertyBatch as $item) {
            ...
            try {
                ...
                if ($response->successful()) {
                    ...
                    if (empty($parcelData['gpin'])) {
                        continue;
                    }

                    $existingParcel = Parcel::where('gpin', $parcelData['gpin'])->first();

                    if ($existingParcel) {
                        $existingParcel->update($parcelData);
                        $updatedCount++;
                        Log::debug("Updated parcel: {$parcelData['gpin']}");
                    } else {
                        $newRecords[] = $parcelData;
                    }
        ...
        if (!empty($newRecords)) {
            Parcel::insert($newRecords);
            Log::info("Inserted " . count($newRecords) . " new parcels");
        }

        if ($updatedCount > 0) {
            Log::info("Updated {$updatedCount} existing parcels");
        }
    }
